...
import cn.org.rapid_framework.generator.util.KeepWords;
...
public class CommandLine {
	...
	private static void processLine(Scanner sc) throws Exception {
		...
		String cmd = sc.next();
		if("gen".equals(cmd)) {
			String[] args = nextArguments(sc);//获取空格后的部分
			if(args.length == 0) return;
			facade.g.setIncludes(getIncludes(args,1));
			facade.generateByTable(args[0],getTemplateRootDir());
			if(SystemHelper.isWindowsOS) {
				System.out.println("outDir:"+GeneratorProperties.getRequiredProperty("outRoot").replace('/', '\\'));
